...
for file in os.listdir(os.path.join(par_path, "musicxml")):
    ...
    for i in range(5):
        new_perf_array = np.zeros((len(note_array)), np.dtype(
            [('beat_period', 'f4'), ('timing', 'f4'), ('articulation_log', 'f4'), ('velocity', 'i4')]))
        ...
        new_perf_array[nm_idx]["velocity"] = perf_array["velocity"].mean().astype("i4")
        new_perf_array[nm_idx]["articulation_log"] = perf_array["articulation_log"].mean()
        # Add noise to the solo match.
        new_perf_array["beat_period"] = new_perf_array["beat_period"] + np.random.normal(0, 0.1, len(part.note_array()))
        new_perf_array["velocity"] = new_perf_array["velocity"] + np.random.randint(-10, 10, len(part.note_array()))
        new_perf_array["articulation_log"] = new_perf_array["articulation_log"] + np.random.normal(0, 0.5, len(part.note_array()))
