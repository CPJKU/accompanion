...
for file in os.listdir(os.path.join(par_path, "musicxml")):
    ...
    for i in range(5):
        ...
        new_perf_array["beat_period"] = mean_beat_period + np.random.normal(0, 0.1, len(part.note_array()))
        new_perf_array["velocity"] = mean_beat_period + np.random.normal(0, 0.1, len(part.note_array()))
        new_perf_array["articulation_log"] = mean_beat_period + np.random.normal(0, 0.5, len(part.note_array()))
