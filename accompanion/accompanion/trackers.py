...
class DummyMultiDTWTracker(object):
    ...
    def __init__(self, score_followers,
    ...
    def __call__(self, frame):
        ...
        score_positions = []
        indices = []
        for sf, strm in zip(self.score_followers,
                            self.state_to_ref_time_maps):
            st = sf(frame)
            indices.append(st)
    ...
    def update_position(self, ref_time):
        for sf, rtsm in zip(self.score_followers,
                            self.ref_to_state_time_maps):
...
class GroundTruthTracker(object):
    ...
    def __init__(self, match_fn, frame_resolution, score_bpm, pipeline, **kwargs):
        ppart, gt_alignment, spart = partitura.load_match(match_fn, create_part=True, first_note_at_zero=True)

        _, ref_onsets, _, _, reference = \
        ...
        # Get matched notes
        matched_idxs = get_matched_notes(reference.note_array, ppart.note_array, gt_alignment)
        ...
        perf_onset_idxs = [np.where(perf_onset_frames == u)[0] for u in unique_perf_onset_frames]

        perf_onset_frames = unique_perf_onset_frames
        ref_onset_frames = np.array([ref_onset_frames[uix].min() for uix in perf_onset_idxs])

        assert len(perf_onset_frames) == len(ref_onset_frames)
        self.input_index = 0
        self.current_position = 0
        self.gt = interpolate.interp1d(perf_onset_frames, ref_onset_frames, kind='linear',
                                       bounds_error=False, fill_value=(ref_onset_frames[0], ref_onset_frames[-1]))
